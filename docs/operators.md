# TopMod Operator Reference

> Generated by `scripts/generate_op_gallery.py` — manual edits will be
> overwritten. Edit the registry in that script and re-run it.

Complete reference for every mesh operator in GenesisTopmod: signature,
parameters, closed-form oracle (element-count effect), tokenizer opcode, and
a before/after visualization. Every operator preserves 2-manifoldness at
every step (the constructive DLFL guarantee of Akleman & Chen 2003).

- The **oracle** column gives the exact closed-form effect on (V, E, F); it
  is what `tests/test_semantic_oracle.py` asserts. All operators preserve χ
  and genus except `add_handle` and crust hole punching.
- The **token** column is the opcode in the `topmod/tokenizer.py` vocabulary;
  tokenized operators can be serialized to integer-ID sequences and replayed
  by `detokenize` (the basis for autoregressive mesh generation).
- All images are rendered by this script; both sides are annotated with
  V/E/F counts.

## Quick Reference

| # | Operator | Token | Oracle (V', E', F') | Image |
|---|---|---|---|---|
| #1 | [`create_vertex`](#1-create_vertex) | CV | V+1, E+0, F+1 (point sphere) | — |
| #2 | [`delete_vertex`](#2-delete_vertex) | — | V−1, E+0, F−1 | — |
| #3 | [`insert_edge`](#3-insert_edge) | IE | E+1; same face → F+1 (split), different faces → F−1 (merge components / open handle) | [img](assets/ops/insert_edge.png) |
| #4 | [`delete_edge`](#4-delete_edge) | DE | E−1; two distinct sides → F−1 (merge), same face both sides → F+1 | [img](assets/ops/delete_edge.png) |
| #5 | [`extrude_face`](#5-extrude_face) | — | V+n, E+2n, F+n (n = face degree) | [img](assets/ops/extrude_face.png) |
| #6 | [`stellate`](#6-stellate) | — | V+1, E+n, F+n−1 | [img](assets/ops/stellate.png) |
| #7 | [`subdivide_edge`](#7-subdivide_edge) | — | V+1, E+1, F+0 | [img](assets/ops/subdivide_edge.png) |
| #8 | [`subdivide_face`](#8-subdivide_face) | — | V+1, E+n, F+n−1 | [img](assets/ops/subdivide_face.png) |
| #9 | [`add_handle`](#9-add_handle) | HDL | V+0, E+n, F+n−2, χ−2, genus+1 (same component) | [img](assets/ops/add_handle.png) |
| #10 | [`stellate_all`](#10-stellate_all) | STA | V'=V+F, E'=3E, F'=2E | [img](assets/ops/stellate_all.png) |
| #11 | [`catmull_clark`](#11-catmull_clark) | CC | V'=V+E+F, E'=4E, F'=2E (all quads) | [img](assets/ops/catmull_clark.png) |
| #12 | [`dual`](#12-dual) | DUAL | V'=F, E'=E, F'=V | [img](assets/ops/dual.png) |
| #13 | [`doo_sabin`](#13-doo_sabin) | DS | V'=2E, E'=4E, F'=V+E+F | [img](assets/ops/doo_sabin.png) |
| #14 | [`simplest_subdivide`](#14-simplest_subdivide) | SIMP | V'=E, E'=2E, F'=F+V | [img](assets/ops/simplest_subdivide.png) |
| #15 | [`vertex_cutting`](#15-vertex_cutting) | VC | V'=2E, E'=3E, F'=F+V | [img](assets/ops/vertex_cutting.png) |
| #16 | [`loop_subdivide`](#16-loop_subdivide) | LOOP | V'=V+E, E'=4E, F'=4F | [img](assets/ops/loop_subdivide.png) |
| #17 | [`sqrt3_subdivide`](#17-sqrt3_subdivide) | SQRT3 | V'=V+F, E'=3E, F'=3F | [img](assets/ops/sqrt3_subdivide.png) |
| #18 | [`honeycomb_subdivide`](#18-honeycomb_subdivide) | HONEY | V'=2E, E'=3E, F'=F+V | [img](assets/ops/honeycomb_subdivide.png) |
| #19 | [`star_subdivide`](#19-star_subdivide) | STAR | V'=V+F+2E, E'=9E, F'=6E (all triangles) | [img](assets/ops/star_subdivide.png) |
| #20 | [`corner_cutting`](#20-corner_cutting) | CCUT | V'=2E, E'=4E, F'=V+E+F (same topology as Doo-Sabin) | [img](assets/ops/corner_cutting.png) |
| #21 | [`loop_style_subdivide`](#21-loop_style_subdivide) | LSTYLE | V'=V+E, E'=4E, F'=F+2E | [img](assets/ops/loop_style_subdivide.png) |
| #22 | [`fractal_subdivide`](#22-fractal_subdivide) | FRAC | V'=V+E+F, E'=6E, F'=4E (all triangles) | [img](assets/ops/fractal_subdivide.png) |
| #23 | [`pentagonal_subdivide`](#23-pentagonal_subdivide) | PENT | V'=V+2E+F, E'=5E, F'=2E (all pentagons) | [img](assets/ops/pentagonal_subdivide.png) |
| #24 | [`pentagonal2_subdivide`](#24-pentagonal2_subdivide) | PENT2 | V'=V+3E, E'=6E, F'=F+2E | [img](assets/ops/pentagonal2_subdivide.png) |
| #25 | [`dual1264_subdivide`](#25-dual1264_subdivide) | D1264 | V'=4E, E'=6E, F'=F+E+V | [img](assets/ops/dual1264_subdivide.png) |
| #26 | [`root4_subdivide`](#26-root4_subdivide) | ROOT4 | V'=V+2E, E'=4E, F'=F+E | [img](assets/ops/root4_subdivide.png) |
| #27 | [`checkerboard_remesh`](#27-checkerboard_remesh) | CHKB | V'=V+4E, E'=9E, F'=F+4E | [img](assets/ops/checkerboard_remesh.png) |
| #28 | [`ds_bc_new_subdivide`](#28-ds_bc_new_subdivide) | DSBC | V'=V+4E, E'=7E, F'=F+2E | [img](assets/ops/ds_bc_new_subdivide.png) |
| #29 | [`dome_subdivide`](#29-dome_subdivide) | DOME | V'=V+59E, E'=116E, F'=F+56E | [img](assets/ops/dome_subdivide.png) |
| #30 | [`create_crust`](#30-create_crust) | CRUST | V'=2V, E'=2E, F'=2F, 2 components; after punching k holes: genus' = 2g+k−1 | [img](assets/ops/create_crust.png) |

---

## 1. Fundamental Operators

### #1 create_vertex

Creates an isolated *point sphere*: a single vertex that forms its own connected component with one degenerate face. It is the starting point of every DLFL construction — all meshes grow from point spheres by inserting edges.

- **Signature**: `create_vertex(mesh, x, y, z) -> Vertex`
- **Token**: `CV`
- **Oracle**: V+1, E+0, F+1 (point sphere)
- **Parameters**: x, y, z — coordinates
- **Example primitive**: cube

```python
v = create_vertex(mesh, 0.0, 0.0, 0.0)
```

*(no image: a single point — nothing meaningful to render)*

### #2 delete_vertex

Removes an isolated point sphere. Only legal on a vertex with no incident edges; it is the exact inverse of `create_vertex`.

- **Signature**: `delete_vertex(mesh, vertex)`
- **Token**: `—`
- **Oracle**: V−1, E+0, F−1
- **Parameters**: —
- **Example primitive**: cube

```python
delete_vertex(mesh, v)
```

*(no image: a single point — nothing meaningful to render)*

### #3 insert_edge

Inserts a new edge between two corners. If both corners lie on the *same* face, the face is split in two (shown: a diagonal chord splits a cube quad into two triangles). If they lie on *different* faces, the two faces merge into one — this is how components are joined and handles are opened. One of the two core DLFL operators; the mesh is a valid 2-manifold after every single call.

- **Signature**: `insert_edge(mesh, he1, he2) -> Edge`
- **Token**: `IE`
- **Oracle**: E+1; same face → F+1 (split), different faces → F−1 (merge components / open handle)
- **Parameters**: he1, he2 — two corners (half-edges)
- **Example primitive**: cube

```python
hes = face.halfedges()
insert_edge(mesh, hes[0], hes[2])  # diagonal across the quad
```

![insert_edge](assets/ops/insert_edge.png)

### #4 delete_edge

Deletes an edge. When the two sides of the edge belong to different faces, those faces merge into one (shown: removing one cube edge merges two squares into a hexagon). The inverse of `insert_edge`.

- **Signature**: `delete_edge(mesh, edge)`
- **Token**: `DE`
- **Oracle**: E−1; two distinct sides → F−1 (merge), same face both sides → F+1
- **Parameters**: —
- **Example primitive**: cube

```python
delete_edge(mesh, edge)
```

![delete_edge](assets/ops/delete_edge.png)

---

## 2. High-Level Operators

### #5 extrude_face

Extrudes a face along its normal, creating a lifted copy of the face (the *top*) connected to the original boundary by n side quads — like pulling a box out of the surface. Returns `[top_face] + side_faces`. The building block of the DOME scheme.

- **Signature**: `extrude_face(mesh, face, dist=1.0) -> List[Face]`
- **Token**: `—`
- **Oracle**: V+n, E+2n, F+n (n = face degree)
- **Parameters**: dist — extrusion distance along the face normal
- **Example primitive**: cube

```python
new_faces = extrude_face(mesh, face, dist=0.6)
top = new_faces[0]
```

![extrude_face](assets/ops/extrude_face.png)

### #6 stellate

Stellates one face: adds an apex vertex at the face centroid (optionally raised along the normal) and connects it to every corner, turning one n-gon into n triangles — a pyramid grown on the face.

- **Signature**: `stellate(mesh, face, dist=0.0) -> Vertex`
- **Token**: `—`
- **Oracle**: V+1, E+n, F+n−1
- **Parameters**: dist — apex displacement along the face normal
- **Example primitive**: cube

```python
apex = stellate(mesh, face)
```

![stellate](assets/ops/stellate.png)

### #7 subdivide_edge

Splits an edge at its midpoint and returns the new midpoint vertex. The two flanking faces each gain one corner; no face is created or destroyed.

- **Signature**: `subdivide_edge(mesh, edge) -> Vertex`
- **Token**: `—`
- **Oracle**: V+1, E+1, F+0
- **Parameters**: —
- **Example primitive**: cube

```python
mid = subdivide_edge(mesh, edge)
```

![subdivide_edge](assets/ops/subdivide_edge.png)

### #8 subdivide_face

Fans a face from its centroid: a center vertex is added and connected to every corner. Topologically identical to `stellate`, but the new vertex stays in the face plane instead of being lifted.

- **Signature**: `subdivide_face(mesh, face) -> Vertex`
- **Token**: `—`
- **Oracle**: V+1, E+n, F+n−1
- **Parameters**: —
- **Example primitive**: cube

```python
c = subdivide_face(mesh, face)
```

![subdivide_face](assets/ops/subdivide_face.png)

### #9 add_handle

Connects two faces of equal degree with a tunnel (handle): both faces are consumed and n side quads bridge their boundaries. This is the only operator that changes genus — a cube becomes a square torus (shown: tunnel between two opposite faces). It is also the hole-punching primitive for `create_crust` shells.

- **Signature**: `add_handle(mesh, face1, face2) -> List[Edge]`
- **Token**: `HDL`
- **Oracle**: V+0, E+n, F+n−2, χ−2, genus+1 (same component)
- **Parameters**: —
- **Example primitive**: cube

```python
add_handle(mesh, top_face, bottom_face)
```

![add_handle](assets/ops/add_handle.png)

### #10 stellate_all

Stellates every face of the mesh at once, producing an all-triangle mesh (a pyramid on every face). Used as a building block inside the honeycomb and star schemes.

- **Signature**: `stellate_all(mesh) -> List[Vertex]  # in place`
- **Token**: `STA`
- **Oracle**: V'=V+F, E'=3E, F'=2E
- **Parameters**: —
- **Example primitive**: cube

```python
apexes = stellate_all(mesh)
```

![stellate_all](assets/ops/stellate_all.png)

---

## 3. Classic Subdivision

### #11 catmull_clark

Catmull-Clark subdivision, the industry-standard smoothing scheme: face points, edge points and repositioned vertex points split every face into quads while pulling the surface toward a smooth limit surface. Output is always an all-quad mesh.

- **Signature**: `catmull_clark(mesh) -> DLFLMesh`
- **Token**: `CC`
- **Oracle**: V'=V+E+F, E'=4E, F'=2E (all quads)
- **Parameters**: —
- **Example primitive**: cube

```python
out = catmull_clark(mesh)
```

![catmull_clark](assets/ops/catmull_clark.png)

### #12 dual

Takes the combinatorial dual: every face becomes a vertex (at its centroid) and every vertex becomes a face. Applying it twice returns the original topology: `dual(dual(M)) ≅ M`. A cube maps to an octahedron and vice versa.

- **Signature**: `dual(mesh) -> DLFLMesh`
- **Token**: `DUAL`
- **Oracle**: V'=F, E'=E, F'=V
- **Parameters**: —
- **Example primitive**: cube

```python
out = dual(mesh)
```

![dual](assets/ops/dual.png)

### #13 doo_sabin

Doo-Sabin subdivision (corner-cutting family): one new vertex per face corner, producing shrunken *face-faces*, quad *edge-faces*, and *vertex-faces* — every sharp corner and edge of the input gets beveled away.

- **Signature**: `doo_sabin(mesh) -> DLFLMesh`
- **Token**: `DS`
- **Oracle**: V'=2E, E'=4E, F'=V+E+F
- **Parameters**: —
- **Example primitive**: cube

```python
out = doo_sabin(mesh)
```

![doo_sabin](assets/ops/doo_sabin.png)

### #14 simplest_subdivide

Mid-edge (simplest / Peters-Reif) subdivision: edge midpoints become the only vertices; each face shrinks to its midpoint polygon and each old vertex is replaced by a new face. A cube becomes a cuboctahedron.

- **Signature**: `simplest_subdivide(mesh) -> DLFLMesh`
- **Token**: `SIMP`
- **Oracle**: V'=E, E'=2E, F'=F+V
- **Parameters**: —
- **Example primitive**: cube

```python
out = simplest_subdivide(mesh)
```

![simplest_subdivide](assets/ops/simplest_subdivide.png)

### #15 vertex_cutting

Vertex truncation: every vertex is sliced off, leaving a small polygon where the corner was, and every n-gon becomes a 2n-gon. A cube becomes a truncated cube.

- **Signature**: `vertex_cutting(mesh, offset=0.25) -> DLFLMesh`
- **Token**: `VC`
- **Oracle**: V'=2E, E'=3E, F'=F+V
- **Parameters**: offset ∈ (0, 0.5) — corner-cut depth
- **Example primitive**: cube

```python
out = vertex_cutting(mesh, offset=0.25)
```

![vertex_cutting](assets/ops/vertex_cutting.png)

### #16 loop_subdivide

Loop subdivision: every triangle is split 1-into-4 at edge midpoints, with β-weighted smoothing of old vertices — the standard smooth scheme for triangle meshes. Raises ValueError on non-triangle input.

- **Signature**: `loop_subdivide(mesh) -> DLFLMesh  # triangle meshes only`
- **Token**: `LOOP`
- **Oracle**: V'=V+E, E'=4E, F'=4F
- **Parameters**: —
- **Example primitive**: icosahedron

```python
out = loop_subdivide(make_icosahedron())
```

![loop_subdivide](assets/ops/loop_subdivide.png)

### #17 sqrt3_subdivide

√3 subdivision (Kobbelt): a vertex is inserted at every face centroid, then all original edges are flipped, tripling the triangle count with the slowest possible growth rate. Raises ValueError on non-triangle input.

- **Signature**: `sqrt3_subdivide(mesh) -> DLFLMesh  # triangle meshes only`
- **Token**: `SQRT3`
- **Oracle**: V'=V+F, E'=3E, F'=3F
- **Parameters**: —
- **Example primitive**: icosahedron

```python
out = sqrt3_subdivide(make_icosahedron())
```

![sqrt3_subdivide](assets/ops/sqrt3_subdivide.png)

---

## 4. TopMod Remeshing Schemes

### #18 honeycomb_subdivide

Honeycomb subdivision, defined as `dual ∘ stellate_all`: stellate every face, then dualize. Triangle input yields a hexagon-dominated (honeycomb-like) mesh.

- **Signature**: `honeycomb_subdivide(mesh) -> DLFLMesh`
- **Token**: `HONEY`
- **Oracle**: V'=2E, E'=3E, F'=F+V
- **Parameters**: —
- **Example primitive**: cube

```python
out = honeycomb_subdivide(mesh)
```

![honeycomb_subdivide](assets/ops/honeycomb_subdivide.png)

### #19 star_subdivide

Star subdivision: `stellate_all` applied twice. A positive offset lifts the first round of apexes along the original face normals, growing star-like spikes on every face.

- **Signature**: `star_subdivide(mesh, offset=0.0)  # in place`
- **Token**: `STAR`
- **Oracle**: V'=V+F+2E, E'=9E, F'=6E (all triangles)
- **Parameters**: offset — first-round apex lift along original face normals
- **Example primitive**: cube

```python
star_subdivide(mesh, offset=0.3)
```

![star_subdivide](assets/ops/star_subdivide.png)

### #20 corner_cutting

Corner-cutting subdivision: a parameterized geometric variant of Doo-Sabin with identical connectivity. `alpha` controls how close each new corner stays to the original corner, i.e. how aggressively corners are shaved off.

- **Signature**: `corner_cutting(mesh, alpha=0.5) -> DLFLMesh`
- **Token**: `CCUT`
- **Oracle**: V'=2E, E'=4E, F'=V+E+F (same topology as Doo-Sabin)
- **Parameters**: alpha ∈ (0, 1) — tension (diagonal weight)
- **Example primitive**: cube

```python
out = corner_cutting(mesh, alpha=0.7)
```

![corner_cutting](assets/ops/corner_cutting.png)

### #21 loop_style_subdivide

Loop connectivity generalized to arbitrary polygons: each face gets its corner triangles cut off, leaving a central midpoint d-gon. On triangle input the connectivity coincides exactly with Loop subdivision.

- **Signature**: `loop_style_subdivide(mesh, length=1.0) -> DLFLMesh`
- **Token**: `LSTYLE`
- **Oracle**: V'=V+E, E'=4E, F'=F+2E
- **Parameters**: length ∈ [0, 1] — old-vertex blend (1 = keep position)
- **Example primitive**: cube

```python
out = loop_style_subdivide(mesh)
```

![loop_style_subdivide](assets/ops/loop_style_subdivide.png)

### #22 fractal_subdivide

Fractal subdivision: `loop_style` followed by stellating every central polygon with an apex raised along the face normal. Repeated application produces a fractal, spiky landscape.

- **Signature**: `fractal_subdivide(mesh, offset=1.0) -> DLFLMesh`
- **Token**: `FRAC`
- **Oracle**: V'=V+E+F, E'=6E, F'=4E (all triangles)
- **Parameters**: offset — spike height factor
- **Example primitive**: cube

```python
out = fractal_subdivide(mesh, offset=1.0)
```

![fractal_subdivide](assets/ops/fractal_subdivide.png)

### #23 pentagonal_subdivide

Pentagonal subdivision: every edge is trisected and every face gets a centroid spoke, converting each d-gon into d pentagons — the whole mesh becomes all-pentagon. A tetrahedron maps to the combinatorial structure of a regular dodecahedron.

- **Signature**: `pentagonal_subdivide(mesh, offset=0.0) -> DLFLMesh`
- **Token**: `PENT`
- **Oracle**: V'=V+2E+F, E'=5E, F'=2E (all pentagons)
- **Parameters**: offset ∈ [0, 1] — pull spoke neighbors toward the centroid
- **Example primitive**: cube

```python
out = pentagonal_subdivide(mesh)
```

![pentagonal_subdivide](assets/ops/pentagonal_subdivide.png)

### #24 pentagonal2_subdivide

Second pentagonal variant: edges are split at midpoints and a scaled inner copy of each face is inserted, then connected — each face becomes one inner d-gon surrounded by d pentagons.

- **Signature**: `pentagonal2_subdivide(mesh, scale_factor=0.75) -> DLFLMesh`
- **Token**: `PENT2`
- **Oracle**: V'=V+3E, E'=6E, F'=F+2E
- **Parameters**: scale_factor — inner-polygon shrink
- **Example primitive**: cube

```python
out = pentagonal2_subdivide(mesh, scale_factor=0.7)
```

![pentagonal2_subdivide](assets/ops/pentagonal2_subdivide.png)

### #25 dual1264_subdivide

Dual 12.6.4 subdivision: Doo-Sabin-like, but each face's inner polygon is a 2d-gon built from the 1/3 and 2/3 points of every edge. Triangle input produces the semi-regular 12.6.4 tiling pattern (dodecagons, hexagons, squares).

- **Signature**: `dual1264_subdivide(mesh, sf=1.0) -> DLFLMesh`
- **Token**: `D1264`
- **Oracle**: V'=4E, E'=6E, F'=F+E+V
- **Parameters**: sf — inner-polygon scale
- **Example primitive**: cube

```python
out = dual1264_subdivide(mesh)
```

![dual1264_subdivide](assets/ops/dual1264_subdivide.png)

### #26 root4_subdivide

Root-4 subdivision: an inner polygon (honeycomb-mask weighted) is inserted in every face and bridged to neighbors with hexagons, while all original edges are deleted. Unlike Doo-Sabin, the original vertices survive.

- **Signature**: `root4_subdivide(mesh, a=0.0, twist=0.0) -> DLFLMesh`
- **Token**: `ROOT4`
- **Oracle**: V'=V+2E, E'=4E, F'=F+E
- **Parameters**: a — old-vertex smoothing blend; twist — inner-ring sampling shift
- **Example primitive**: cube

```python
out = root4_subdivide(mesh, a=0.3, twist=0.2)
```

![root4_subdivide](assets/ops/root4_subdivide.png)

### #27 checkerboard_remesh

Checkerboard remeshing: each face is inset, each edge trisected, and corners are chamfered, producing an alternating quad pattern. Quad input stays all-quad, with a visible checkerboard layout.

- **Signature**: `checkerboard_remesh(mesh, thickness=0.25) -> DLFLMesh`
- **Token**: `CHKB`
- **Oracle**: V'=V+4E, E'=9E, F'=F+4E
- **Parameters**: thickness ∈ (0, 0.5) — inset / trisection ratio
- **Example primitive**: cube

```python
out = checkerboard_remesh(mesh, thickness=0.25)
```

![checkerboard_remesh](assets/ops/checkerboard_remesh.png)

### #28 ds_bc_new_subdivide

Doo-Sabin "BC new" variant: a Doo-Sabin pass is applied to the midpoint-refined 2d-gon boundary of every face, but the original vertices survive. Each face yields one 2d-gon plus two pentagons per edge.

- **Signature**: `ds_bc_new_subdivide(mesh, sf=1.0, length=1.0) -> DLFLMesh`
- **Token**: `DSBC`
- **Oracle**: V'=V+4E, E'=7E, F'=F+2E
- **Parameters**: sf — DS corner scale; length — old-vertex blend
- **Example primitive**: cube

```python
out = ds_bc_new_subdivide(mesh, sf=0.9)
```

![ds_bc_new_subdivide](assets/ops/ds_bc_new_subdivide.png)

### #29 dome_subdivide

Dome subdivision: every edge is split into quarters, then every original face is extruded seven times with a built-in height/scale profile, growing a rounded dome on each face — the mesh sprouts a bubble on every side.

- **Signature**: `dome_subdivide(mesh, length=1.0, sf=1.0)  # in place`
- **Token**: `DOME`
- **Oracle**: V'=V+59E, E'=116E, F'=F+56E
- **Parameters**: length — height profile scale; sf — ring scale profile
- **Example primitive**: cube

```python
dome_subdivide(mesh)
```

![dome_subdivide](assets/ops/dome_subdivide.png)

---

## 5. Structural Operators

### #30 create_crust

Turns a surface into a hollow shell: the whole mesh is duplicated with reversed orientation and offset inward along averaged vertex normals, giving an outer and an inner wall. Returns the list of mirrored face pairs (outer face i ↔ inner face F+i); punching holes through pairs with `add_handle` connects the walls and creates tunnels (shown: shell with one hole punched).

- **Signature**: `create_crust(mesh, thickness=0.1) -> (DLFLMesh, pairs)`
- **Token**: `CRUST`
- **Oracle**: V'=2V, E'=2E, F'=2F, 2 components; after punching k holes: genus' = 2g+k−1
- **Parameters**: thickness — shell thickness (negative offsets outward)
- **Example primitive**: cube

```python
out, pairs = create_crust(mesh, thickness=0.25)
for outer, inner in pairs[:2]:
    add_handle(out, outer, inner)  # each hole: genus +1
                                   # (first hole joins the walls)
```

![create_crust](assets/ops/create_crust.png)

## Tokenizer Usage

```python
from topmod import tokenize, detokenize, build_vocabulary, encode_sequence
from topmod.tokenizer import TopModToken

# Every token sequence executes from the deterministic icosahedron primitive
tokens = [TopModToken(op='PENT'), TopModToken(op='DUAL'), TopModToken(op='EOS')]
mesh = detokenize(tokens)            # guaranteed: is_manifold(mesh) == True

vocab = build_vocabulary()           # token → integer ID (append-only)
ids = encode_sequence(tokens, vocab)
```

Holed shells (a genus construction that generative models can learn):

```python
# After CRUST the mirrored face pairs have deterministic ordinals:
# outer face i ↔ inner face F+i, so hole punching is expressible with the
# existing HDL(face1, face2) token.
tokens = [TopModToken(op='CRUST'),
          TopModToken(op='HDL', refs=(0, 20)),   # icosahedron: F=20
          TopModToken(op='EOS')]
```

## Testing

```bash
python3 -m pytest tests/ -q --ignore=tests/test_manifold_loss.py --ignore=tests/test_pipeline.py
```

Per-operator oracle tests live in `tests/test_semantic_oracle.py` (four
primitives × exact ΔV/ΔE/ΔF/χ/genus + face-degree census); token tests in
`tests/test_tokenizer.py`.

## References

- Akleman & Chen 2003 — the minimal complete DLFL operator set
- `docs/reference_semantics.md` — clean-room extraction (with χ
  verification) of the reference library's semantics (davyrisso/topmod3d, GPL)
- `docs/vocabulary_roadmap.md` — vocabulary evolution roadmap
