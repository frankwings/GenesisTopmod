...
chain() { S=$1; T0=$(date +%s); T=${S}_$P
  ...
  echo "##### $S: golden v3 chain ($P, seed $SEED), GT genus ${GT[$S]}"
  ...
  if [ -f $O/cow_${S}_${T}_p4.npz ]; then echo "[$S 5] skip"; else guard; env SNAPSHOT_TITLE="Stage 5 [golden v3 loop]" MODE=64v SHAPE=$S TAG=${T}_p4 $PALF BASE_NPZ=$O/cow_${T}_early.npz python3 -u despike/phase4_inloop.py 2>&1 | grep --line-buffered "$F4" | sed "s/^/[$S 5] /"; fi
  ...
  if [ -f $O/cow_${T}_p4g.npz ]; then echo "[$S 5b] skip"; else guard
    env SNAPSHOT_TITLE="Stage 5b [late genus pass]" MODE=64v SHAPE=$S TAG=${T}_p4g MAX_HANDLES=4 GENUS_TARGET=hull BASE_NPZ=$O/cow_${S}_${T}_p4.npz python3 -u despike/phase7_handle.py 2>&1 | grep --line-buffered "genus target\|after handle\|contact\|no more\|UNREACHED\|handles added\|broke\|Traceback\|Error" | sed "s/^/[$S 5b] /"
    cp $O/cow_${S}_${T}_p4g.npz $O/cow_${T}_p4g.npz; fi
  echo "[$S 5b] genus after late pass: $(genus_of $O/cow_${T}_p4g.npz) (GT ${GT[$S]})"
  ...
  G=$(genus_of despike/results_genus/${T}_auto.npz); echo "[RESULT] $S $P: final genus $G (GT ${GT[$S]}) $([ "$G" = "${GT[$S]}" ] && echo OK || echo MISMATCH)"
  guard; SHAPE=$S python3 despike/eval_cd_iou.py raw=despike/results_genus/${T}_raw.npz taubin=despike/results_genus/${T}_auto.npz 2>&1 | grep -E "cd_iou|Error" | sed "s/^/[$S exam] /"
