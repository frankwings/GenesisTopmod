...
## 有洞物体：把手阶段的最终形态（2026-09-04）
`phase7_handle.py`（DETECT=rays, OPEN=merge）+ `phase7_multi.sh`（一轮一个洞）：
1. **证据**：训练视角剪影里被前景包围的背景像素（2D 洞）→ 这条视线上证明没有物体；我们的渲染若在此为前景，视线的进入面/穿出面就是堵隧道的膜。按洞面积排序，一轮取最大的一个。
2. **判膜**：壳外面片的连通块必须是拓扑圆盘（χ=1）；插过管的膜是环带（χ=0），视线打在壳上的面不是膜。这就是"一条隧道一个把手"的判据，不用距离阈值。
3. **切膜**：膜内部顶点 `collapse_edge_tri` 到膜边缘 → `delete_edge` 删膜内边 → 膜变成一个多边形 → 两个多边形顶点数对齐（`subdivide_edge`）、起点对齐 → `add_handle`。管壁直接横跨隧道壁，不靠优化撑开。
4. 兜底：合并失败（薄片正反面共享顶点）回退到单面把手；破坏流形则拉黑该 2D 洞；每轮后 400 步循环。

| 形状 | GT genus | 把手数 | 结果 genus | ho16 | hair |
|---|---|---|---|---|---|
...
## Stage 6 平滑强度（2026-09-04 复核）
按局部厚度自适应（细结构少平）**没有帮助**：fertility 的损失不在细胳膊，而是网格偏粗（平均边 0.049）时 5 次 Taubin 磨掉了真实特征。
ho16 随迭代次数：

| | ×1 | ×2 | ×3 | ×5 |
|---|---|---|---|---|
| armadillo | 0.9948 | 0.9960 | **0.9962** | 0.9957 |
| rocker-arm | — | **0.9975** | 0.9975 | 0.9972 |
| 3holes | 0.9955 | 0.9958 | 0.9958 | **0.9959** |
| fertility | **0.9890** | 0.9885 | 0.9873 | 0.9847 |

结论：×2–×3 对所有形状都在最优 0.0005 以内；×5 对粗网格过度。`phase5_taubin.py AUTO=1`：按 64 张**训练**视角的 IoU 选迭代数
（不碰考试视角），并设下限 AUTO_MIN=2（训练 IoU 偏好欠平滑——它奖励贴合训练剪影的抖动）。四个形状 AUTO 都选 2。
