"""V15: v13 recipe with two root-cause fixes for the shoulder-fin needle:
...
def optimize_phase(ctx, verts_np, tris_np, gt, gtd, mvps, steps, label,
    ...
    for step in range(steps):
        ...
        if tmask is not None and tanchor is not None and tmask.any():
            ...
            adist = (verts_t[tmask] - tanchor).norm(dim=-1)
            loss = loss + W_TUBE * F.relu(adist - 0.5 * me).pow(2).sum() / nv
        loss.backward()
...
def main():
    ...
    with open(f"{OUT}/cow_v15.obj", "w") as fh:
        for x, y, z in v: fh.write(f"v {x} {y} {z}\n")
        for a, b, c in t: fh.write(f"f {a+1} {b+1} {c+1}\n")
    np.save(f"{OUT}/v15_tubemask.npy", tm)
    print(f"saved {OUT}/cow_v15.obj", flush=True)
    ho_iou, ho_hair, ho_mb = heldout_exam(ctx, v, t)
    print(f"\n=== V15 RESULT (deadzone anchor-pull from cc2) ===")
    print(f"train6 IoU={iou:.4f}")
    print(f"heldout16: IoU={ho_iou:.4f} hair_px={ho_hair} maxblob={ho_mb}")
    print(f"refs: v14 0.6443(runaway); v13 train6=0.9754 ho=0.8981/40048/2898; v10 0.9827/0.8714")
