...
|---|---|---|---|
| 1 sphere → coarse | `run_64v.py STOP_AFTER=cc3` | icosphere 320 f; cc2 800 steps; midpoint subdivide (numpy); cc3 800 steps. Each step: 64-view nvdiffrast (sil + depth + headlight diffuse) → L1 + Laplacian + edge + quality + spike/sliver/fold/tube regs → Adam | none |
...
| 4 cc4 + despike | `run_64v.py RESUME_FROM` | midpoint subdivide → cc4 800 steps → surgery 1 → settle 400 → surgery 2 | `collapse_edge_tri` (amputation) |
...
- TopMod DLFL: stages 2, 3, 4 (surgery), 5, 6. Operators used across the chain: `collapse_edge_tri`, `delete_edge`, `insert_edge`, `subdivide_edge`, `stellate`, `add_handle`.
- Non-TopMod topology change: the three midpoint subdivisions in Stage 1/4 (uniform 1→4, numpy) — could be replaced by DLFL `subdivide_edge` + `stellate`; kept as-is because uniform midpoint on a closed triangle mesh is trivially manifold.
