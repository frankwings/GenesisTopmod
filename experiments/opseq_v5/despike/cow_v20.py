"""V20: v17 blanket tube prior with THICKNESS discrimination.
Diagnosis: v17's ears were blunted DURING TRAINING (pre-surgery already lacks
the ear tip) by the blanket tube prior at TUBE_THR=0.4, which flags the ear
(front-back gap < 0.4*mean_edge) as a thin tube and pulls it toward centroid.
A needle is THINNER than an ear, so lowering TUBE_THR catches only genuine
near-degenerate needles and spares the thicker ear/horn sheets.
No smart gate (v18 IoU-gate and v19 fold-gate both fail-open and resurrect the
needle). Just the right thickness threshold. Supervision = 6 training views.
"""
import sys, os
...
import cow_v13
cow_v13.TUBE_THR = float(os.environ.get("TUBE_THR", "0.25"))
print(f"[v20] TUBE_THR = {cow_v13.TUBE_THR}", flush=True)
...
def main():
    ...
    v, t = surgery(np.asarray(v, np.float64), np.asarray(t, np.int64),
                   iou_fn=iou_fn, iou_budget=3e-4, global_cap=1.5e-3, max_rounds=8, max_grow=4)
    ...
    v, t = surgery(np.asarray(v, np.float64), np.asarray(t, np.int64),
                   iou_fn=iou_fn, iou_budget=2e-4, global_cap=6e-4, max_rounds=4, max_grow=4)
    ...
    cols = [("GT cow", gv, gf), ("v17 (thr0.4)", z17["verts"], z17["tris"]), (f"{TAG} (thr{cow_v13.TUBE_THR})", v, t)]
