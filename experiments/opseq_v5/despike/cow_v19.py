"""V19: v17 + GEOMETRY-AWARE tube prior ("save the ears, kill the needles").
v18's IoU-based gate failed: the shoulder needle DOES lower 6-view loss (it
was born to fit those silhouettes), so the gate mistook it for an ear and it
came back (ho16 0.9066, hair 34362).
The true discriminator is GEOMETRIC DEGENERACY, not silhouette support:
  - a needle/fin is a SELF-FOLDED sheet: adjacent faces bend ~180deg
    (their normals are near-antiparallel)  -> PULL it in
  - an ear/horn is an UNFOLDED flat sheet: adjacent faces bend ~0deg
    -> EXEMPT from pull
Every TUBE_EVERY steps we split the thin mask into comps and pull only comps
that contain a near-antiparallel face pair.  Surgery passes are gentle and
the final light pass runs AFTER settle.  Supervision = the 6 training views.
"""
import sys, os, time
...
FOLD_COS = -0.5   # adjacent-face normal dot below this => self-fold (needle)


def build_vface(tris_np, nv):
...
@torch.no_grad()
def gate_fold(tmask, verts, faces_l, pairs_t, adj_np, vface):
    """Pull only thin comps that are SELF-FOLDED (contain a near-antiparallel
    adjacent-face pair). Flat sheets (ears/horns) are exempt. Geometry only —
    no silhouette used, so a silhouette-supported needle is still caught."""
    flagged = tmask.cpu().numpy()
    if not flagged.any():
        return tmask
    # per-face normals + per-adjacent-pair normal dot
    tri = verts[faces_l]
    n = torch.cross(tri[:, 1] - tri[:, 0], tri[:, 2] - tri[:, 0], dim=-1)
    n = n / (n.norm(dim=-1, keepdim=True) + 1e-12)
    pd = (n[pairs_t[:, 0]] * n[pairs_t[:, 1]]).sum(-1)  # per-pair cos
    folded_face = torch.zeros(len(faces_l), dtype=torch.bool, device=verts.device)
    fp = pairs_t[pd < FOLD_COS]
    folded_face[fp[:, 0]] = True; folded_face[fp[:, 1]] = True
    folded_face_np = folded_face.cpu().numpy()
    ...
    for i in np.where(flagged)[0]:
        ...
        # does this comp touch a folded face?
        self_folded = any(folded_face_np[f] for v_ in comp for f in vface[v_])
        if self_folded:
            keep[torch.tensor(comp, device=verts.device)] = True
    return keep
...
def optimize_phase(ctx, verts_np, tris_np, gt, gtd, mvps, steps, label,
    ...
    pairs_t = torch.tensor(build_pairs(tris_np), device=DEVICE)  # always (gate)
    vface = build_vface(tris_np, len(verts_np))
    ...
    for step in range(steps):
        ...
        if use_tube and step % TUBE_EVERY == 0:
            ...
            tmask = gate_fold(raw, verts_t.detach(), faces_l, pairs_t,
                              adj_np, vface)
        opt.zero_grad()
        ...
        if use_fold and step >= FOLD_START:
...
def main():
    ...
    save_obj(f"{OUT}/cow_v19_pre.obj", v, t)
    ...
    save_obj(f"{OUT}/cow_v19.obj", v, t)
    np.savez(f"{OUT}/cow_v19.npz", verts=v, tris=t)
    ho_iou, ho_hair, ho_mb = heldout_exam(ctx, v, t, scene)
    print(f"\n=== V19 RESULT (fold-gated tube prior) ===")
    ...
    plt.tight_layout()
    plt.savefig(f"{OUT}/cow_v19_compare.png", bbox_inches="tight")
    ...
    plt.tight_layout()
    plt.savefig(f"{OUT}/cow_v19_360.png", bbox_inches="tight")
    print("saved cow_v19_compare.png, cow_v19_360.png", flush=True)
