"""V22: GT-mismatch SEED + thickness EXTENT, component-propagated.

Boss's plan (verified sound):
  1. Find the 2D pixels where pred disagrees with GT -> but only the RED kind
     (pred-outside-GT), i.e. the needle TIP poking out.  These map back to
     `escape` seed vertices.
  2. Use the v20 thickness rule to define the removal EXTENT: the whole needle
     is thin end-to-end.  PROPAGATE the tip seed over the thin-connected
     component -> condemn the WHOLE buried shaft (root and all).
     The ear is thin too, but its component has NO escaping tip -> spared.

Difference from v21 (which FAILED): v21 did per-vertex `thin & escape`, so it
only ever removed the tip (195 verts) and left the shaft, which regrew.  v22
propagates the seed along the thin component so the whole needle goes.

TUBE_THR back to 0.4 (blanket thin): escape+propagation supplies the
discrimination, not the threshold.  Supervision = 6 training views only.
"""
import sys, os
...
print(f"[v22] TUBE_THR = {cow_v13.TUBE_THR} (escape-seed + thin-propagate)", flush=True)
...
TAG = os.environ.get("TAG", "v22")
DILATE = int(os.environ.get("DILATE", "2"))

_MVPS = None
_GT = None
_CUR_ADJ = None          # adjacency dict for the faces currently being optimized
_orig_tube_mask = cow_v13.tube_mask


def _set_faces(F):
    """Build 1-hop adjacency for the current (topology-constant) phase."""
    global _CUR_ADJ
...
def _tube_mask_gated(v, excl, mean_edge):
    """thin component that contains an escaping tip (v22 propagation).
    v is a torch tensor [V,3] on DEVICE."""
...
def main():
    ...
    print(f"\n=== {TAG.upper()} RESULT (TUBE_THR={cow_v13.TUBE_THR}, seed+propagate) ===")
    print(f"train6 IoU={iou_final:.4f} (post-train {iou_train:.4f})")
    print(f"heldout16: IoU={ho_iou:.4f} hair_px={ho_hair} maxblob={ho_mb}")
    print(f"refs: v20 train6=0.9789 ho=0.9438/9544 (thr0.25) | v21 ho=0.9091/32956 (B)")
