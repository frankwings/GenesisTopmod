#!/usr/bin/env python3
"""
gen_data_v4.py — Generate the opseq_v4 classification dataset.

Replaces the sequence prediction dataset (v3) with a simple N-class
classification dataset. For each sample:
  1. Pick a random canonical topology (uniform over 20 classes)
  2. Build the mesh via build_topo_mesh()
  3. Apply random deformations (anisotropic scale, rotation, bumps, twist)
  4. Normalize to 80% of [-2, +2]
  5. Render 4-view silhouettes (128x128, BLACK object on WHITE background)
  6. Save: images [N,4,128,128] uint8, labels [N] int8
...
Usage:
    python gen_data_v4.py [--n_train 50000] [--n_val 5000] [--seed 42]
                          [--out_dir experiments/opseq_v4/data]
...
# ═════════════════════════════════════════════════════════════════════════════
# Vertex deformations (copied from gen_data_v3.py)
# ═════════════════════════════════════════════════════════════════════════════
...
def generate_sample_v4(
    ...
    """
    Generate one v4 (images, class_label) training sample.
    ...
    for i in range(4):
        ...
        # WHITE background: 255=bg, 0=fg  (invert: render gives 1=fg, 0=bg)
...
def save_shard_v4(
    ...
    labels = np.array(labels_list, dtype=np.int8)        # [N]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate opseq_v4 classification dataset")
    ...
    for split_name, indices in splits.items():
        ...
        for sample_idx, global_idx in enumerate(indices):
            ...
            try:
                images, label = generate_sample_v4(rng, ctx, device=args.device)
            ...
            if len(images_buf) >= shard_size:
                ...
                save_shard_v4(images_buf, labels_buf, path)
                images_buf, labels_buf = [], []
                shard_idx += 1
    ...
    print("V4 classification dataset generation complete.")
